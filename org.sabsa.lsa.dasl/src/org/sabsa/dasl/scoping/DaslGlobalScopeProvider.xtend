...
class DaslGlobalScopeProvider extends ImportUriGlobalScopeProvider {
	...
	override protected getImportedUris(Resource resource) {
		return cache.get(DaslGlobalScopeProvider.getSimpleName(), resource, new Provider<LinkedHashSet<URI>>() {
			override get() {
			...
			def LinkedHashSet<URI> collectImportUris(Resource resource, LinkedHashSet<URI> uniqueImportURIs) {
				...
				val models = resourceDescription.getExportedObjectsByType(DaslPackage.Literals.SECURITY_MODEL)
				
				models.forEach[
					val userData = getUserData(DaslResourceDescriptionStrategy.IMPORTS)
					if(userData !== null) {
